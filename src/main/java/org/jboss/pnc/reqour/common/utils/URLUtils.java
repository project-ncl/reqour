...
public class URLUtils {
    ...
    public static ExternalURLValidator.ParsedURL parseURL(String url) {
        return ExternalURLValidator.parseURL(url);
    }

    public static String addUsernameToUrl(String url, String username) {
        if (username == null || isScpLike(url)) {
            return url;
        }

        ExternalURLValidator.ParsedURL parsedURL = parseURL(url);
        if (parsedURL.getUser() != null) {
            return url;
        }

        return composeURL(
                ExternalURLValidator.ParsedURL.builder()
    ...
    private static boolean isScpLike(String url) {
        return Patterns.ScpLike.PATTERN.matcher(url).matches();
    }

    private static String composeURL(ExternalURLValidator.ParsedURL parsedURL) {
