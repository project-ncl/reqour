...
import org.jboss.pnc.reqour.common.validation.ValidationGroups;
import org.jboss.pnc.reqour.dto.ExternalToInternalDTO;
...
public interface TranslationEndpoint {
    ...
    @APIResponses({ @APIResponse(
            ...
            content = @Content(schema = @Schema(implementation = ExternalToInternalDTO.class))) })
    @POST
    @Path("/external-to-internal")
    ExternalToInternalDTO externalToInternal(
            @Valid @ConvertGroup(
                    to = ValidationGroups.WhenRequesting.class) ExternalToInternalDTO externalToInternalDto);
}
